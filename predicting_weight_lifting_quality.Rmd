...
nz_vars <- nearZeroVar(select(training,-one_of(window_id)),saveMetrics = T)
```
...
mod_lda <- train(classe~.,method = "lda",
                   data = select(training,classe,
                                 one_of(selected_features)))
tf4 <- difftime(Sys.time(),ti)

ti <- Sys.time()
set.seed(3676)
mod_nb <- train(classe~.,method = "nb",
                   data = select(training,classe,
                                 one_of(selected_features)))
tf5 <- difftime(Sys.time(),ti)

# saving the models
save(mod_boost,mod_rf,mod_lda,mod_nb,file = "fitted_models.rda")
```
...
tb_lda <- 
testing[,100*prop.table(table(Observed = classe,Predicted = pred_lda),2)]

tb_nb <- 
testing[,100*prop.table(table(Observed = classe,Predicted = pred_nb),2)]
...
ti <- Sys.time()
set.seed(3676)
mod_boost_final <- train(classe~.,method = "gbm",
                         data = select(pml_data_transf,
                                       classe,
                                       one_of(selected_features)))
tf2 <- difftime(Sys.time(),ti)
```
...
validate_transf[,`:=`(pred_boost = predict(mod_boost,
                                           newdata = validate_transf),
                      pred_boost_final = predict(mod_boost_final,
                                                 newdata = validate_transf),
                      pred_rf = predict(mod_rf,
                                        newdata = validate_transf),
                      pred_lda = predict(mod_lda,
                                         newdata = validate_transf))]
View(validate_transf[,list(V1,pred_boost,pred_boost_final,pred_rf,pred_lda)])
```
